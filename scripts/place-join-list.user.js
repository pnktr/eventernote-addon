// ==UserScript==
// @name         Eventernote 会場別参加イベント一覧
// @namespace    https://www.eventernote.com/
// @version      0.1.0
// @description  指定した会場で開催された参加イベントを一覧表示する
// @author       pinokotere
// @match        https://www.eventernote.com/places/*
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    container.innerHTML = `
        ...
        <div style="margin-top:8px">
            <label>会場: <span id="en_place_name" style="font-weight:bold"></span></label>
        </div>
    ...
    const placeName = document.querySelector('h1')?.textContent.trim().replace('のイベント・ライブ会場情報', '').trim() ?? '不明';
    document.getElementById('en_place_name').textContent = placeName;
        ...
        for (let year = startYear; year <= endYear; year++) {
            ...
            while (hasNext) {
                ...
                events.forEach(event => {
                    const placeHref = event.querySelector('.place a')?.getAttribute('href');
                    const placeIdInEvent = placeHref?.split('/')[2];
                    if (placeIdInEvent === placeId) {
                        ...
                        const actors = [...event.querySelectorAll('.actor ul li a')].map(a => a.textContent.trim()).join(', ');
                        results.push({ date, name, eventUrl, actors });
                    }
                });
